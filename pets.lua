--[[
    Script: Pet Mutation Bot (Alur Equip & Jeda)
    Pembaruan:
    - Menambahkan perintah EquipTool untuk memegang pet sebelum dimasukkan ke mesin.
    - Menambahkan jeda 20 detik setelah mengklaim pet dari mesin.
    - Menghapus variabel yang tidak diperlukan untuk optimasi.
]]
...
-- Diubah agar mengembalikan seluruh objek Tool, bukan hanya UUID
function FindPetToPlaceOnFarm()
    ...
    for _, item in ipairs(backpack:GetChildren()) do
        if item:IsA("Tool") and item:GetAttribute("b") == "l" and string.find(item.Name, "Dilophosaurus") then
            return item -- Kembalikan objek Tool
        end
    end
    return nil
end
...
if not success then
    ...
    coroutine.wrap(function()
        while task.wait(1) do
            ...
            local petOnFarmUUID = FindPetForMutation()
            
            GUI.timerLabel.Text="Timer Mesin: "..timerText
            GUI.targetLabel.Text="Target Pet: "..(petOnFarmUUID or "Tidak ada")
            GUI.cyclesLabel.Text="Siklus Selesai: "..cyclesCompleted
            
            if botActive then
                if petOnFarmUUID and timerText:upper() == "READY" then
                    botActive = false
                    ...
                    if petModelLocation:FindFirstChild("Dilophosaurus") then
                        ...
                        -- Jeda 20 detik setelah klaim
                        print("Menunggu 20 detik...")
                        task.wait(20) 
                    end
                    
                    print("Mengambil pet dari farm (Unequip)...")
                    petsServiceEvent:FireServer("UnequipPet", petOnFarmUUID)
                    task.wait(3)
                    
                    -- Langkah baru: Cari pet di backpack untuk di-equip
                    print("Mencari pet di backpack untuk dipegang...")
                    local petToEquip = FindPetToPlaceOnFarm()
                    
                    if petToEquip then
                        print("Memegang pet: " .. petToEquip.Name)
                        local humanoid = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
                        if humanoid then
                            -- Equip pet agar dipegang oleh karakter
                            humanoid:EquipTool(petToEquip)
                            task.wait(2) -- Beri jeda agar animasi equip selesai
                            
                            print("Memasukkan pet yang dipegang ke mesin...")
                            ...
                            print("Siklus selesai.")
                             ...
                             warn("Humanoid tidak ditemukan, tidak bisa memegang pet.")
                        end
                    else
                        warn("Tidak ditemukan pet 'Dilophosaurus' di backpack untuk dimasukkan ke mesin.")
                    end
