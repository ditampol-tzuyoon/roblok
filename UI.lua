...
function RayfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Ohdear"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Ohdear Interface Suite" then
		LoadingFrame.Version.Text = "Ohdear UI"
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			-- PERKECIL UKURAN FONT TITLE
			Dropdown.Title.TextSize = 11
			Dropdown.Selected.TextSize = 10
			Dropdown.List.Visible = false
			
			-- Pastikan CurrentOption selalu array
			if DropdownSettings.CurrentOption then
				if type(DropdownSettings.CurrentOption) == "string" then
					DropdownSettings.CurrentOption = {DropdownSettings.CurrentOption}
				end
				-- Filter hanya options yang valid
				local validOptions = {}
				for _, opt in ipairs(DropdownSettings.CurrentOption) do
					if table.find(DropdownSettings.Options, opt) then
						table.insert(validOptions, opt)
					end
				end
				DropdownSettings.CurrentOption = validOptions
			else
				DropdownSettings.CurrentOption = {}
			end

			-- Function untuk update teks yang dipilih
			local function updateSelectedText()
				if #DropdownSettings.CurrentOption == 0 then
					Dropdown.Selected.Text = "Tidak ada yang dipilih"
				elseif #DropdownSettings.CurrentOption == 1 then
					Dropdown.Selected.Text = DropdownSettings.CurrentOption[1]
				elseif #DropdownSettings.CurrentOption == #DropdownSettings.Options then
					Dropdown.Selected.Text = "Semua terpilih"
				else
					Dropdown.Selected.Text = #DropdownSettings.CurrentOption .. " item terpilih"
				end
			end
			...
			Dropdown.Toggle.Size = UDim2.new(0, 14, 0, 14)
			Dropdown.Toggle.Position = UDim2.new(1, -18, 0.5, -7)
			...
			SearchInput.PlaceholderText = "Cari..."
			SearchInput.ClearTextOnFocus = false
			SearchInput.Text = ""
			SearchInput.Size = UDim2.new(1, -10, 0, 22)
			SearchInput.TextSize = 11
			SearchInput.Position = UDim2.new(0, 5, 0, 5)
			SearchInput.ZIndex = 10
			...
			local function filterDropdownOptions(searchText)
				for _, option in ipairs(Dropdown.List:GetChildren()) do
					if option.ClassName == "Frame" and option.Name ~= "Placeholder" and option.Name ~= "SearchInput" then
						if string.lower(option.Name):find(string.lower(searchText), 1, true) or searchText == "" then
							option.Visible = true
						else
			...
			SearchInput:GetPropertyChangedSignal("Text"):Connect(function()
				filterDropdownOptions(SearchInput.Text)
			end)
			...
			SearchInput.FocusLost:Connect(function(enterPressed)
				if enterPressed then
					...
					for _, option in ipairs(Dropdown.List:GetChildren()) do
						if option.ClassName == "Frame" and option.Visible and option.Name ~= "Placeholder" and option.Name ~= "SearchInput" then
							table.insert(visibleOptions, option)
						end
					end
					
					if #visibleOptions > 0 then
						visibleOptions[1].Interact:Fire()
					end
				end
			end)

			-- Hapus options yang tidak digunakan
			...
			-- Function untuk membuat options dropdown
			local function SetDropdownOptions()
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Title.TextSize = 11
					DropdownOption.Size = UDim2.new(1, -10, 0, 20)

					-- Tambah checkbox indicator
					local Checkbox = Instance.new("Frame")
					Checkbox.Name = "Checkbox"
					Checkbox.Size = UDim2.new(0, 12, 0, 12)
					Checkbox.Position = UDim2.new(0, 5, 0.5, -6)
					Checkbox.BackgroundColor3 = SelectedTheme.ToggleBackground
					...
					Checkmark.Image = "rbxassetid://10734947220"
					Checkmark.ImageColor3 = SelectedTheme.ToggleEnabled
					Checkmark.Visible = false
					
					local UIStroke = Instance.new("UIStroke")
					UIStroke.Color = SelectedTheme.ToggleDisabledStroke
					UIStroke.Thickness = 1
					UIStroke.Parent = Checkbox
					
					Checkmark.Parent = Checkbox
					Checkbox.Parent = DropdownOption
					
					-- Adjust title position
					DropdownOption.Title.Position = UDim2.new(0, 22, 0.5, 0)
					DropdownOption.Title.Size = UDim2.new(1, -27, 1, 0)
					
					-- Set checkbox state berdasarkan selection
					if table.find(DropdownSettings.CurrentOption, Option) then
						Checkmark.Visible = true
						Checkbox.BackgroundColor3 = SelectedTheme.ToggleEnabled
						UIStroke.Color = SelectedTheme.ToggleEnabledStroke
					end
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						local checkbox = DropdownOption:FindFirstChild("Checkbox")
						...
						if table.find(DropdownSettings.CurrentOption, Option) then
							...
							if checkmark then
								checkmark.Visible = false
								checkbox.BackgroundColor3 = SelectedTheme.ToggleBackground
								checkbox.UIStroke.Color = SelectedTheme.ToggleDisabledStroke
							end
						else
							-- Tambah ke selection
							...
							if checkmark then
								checkmark.Visible = true
								checkbox.BackgroundColor3 = SelectedTheme.ToggleEnabled
								checkbox.UIStroke.Color = SelectedTheme.ToggleEnabledStroke
							end
						end

						updateSelectedText()

						-- Callback selalu menerima array
						local Success, Response = pcall(function()
			...
			-- Set ukuran option yang lebih kecil
			for _, droption in ipairs(Dropdown.List:GetChildren()) do
				if droption.ClassName == "Frame" and droption.Name ~= "Placeholder" and droption.Name ~= "SearchInput" then
					droption.Size = UDim2.new(1, -10, 0, 22) -- Diperkecil
					droption.BackgroundColor3 = SelectedTheme.DropdownUnselected
					
					-- Pastikan title position correct
					if droption:FindFirstChild("Title") then
						droption.Title.Position = UDim2.new(0, 25, 0.5, 0)
						droption.Title.Size = UDim2.new(1, -30, 1, 0)
						droption.Title.TextSize = 11
					end
			...
			Dropdown.Interact.MouseButton1Click:Connect(function()
				...
				if Dropdown.List.Visible then
					Debounce = true
					TweenService:Create(Dropdown, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 40)}):Play() -- Diperkecil
					for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
						if DropdownOpt.ClassName == "Frame" and DropdownOpt.Name ~= "Placeholder" and DropdownOpt.Name ~= "SearchInput" then
					...
					-- Hitung tinggi berdasarkan jumlah options yang visible (maksimal 6 options)
					local visibleOptions = 0
					for _, option in ipairs(Dropdown.List:GetChildren()) do
						if option.ClassName == "Frame" and option.Name ~= "Placeholder" and option.Name ~= "SearchInput" and option.Visible then
							visibleOptions = visibleOptions + 1
						end
					end
					
					local maxHeight = math.min(visibleOptions, 6) * 22 + 30
					TweenService:Create(Dropdown, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, maxHeight)}):Play()
			...
			-- Function Refresh untuk dropdown
			function DropdownSettings:Refresh(optionsTable, keepSelections)
				DropdownSettings.Options = optionsTable
				
				-- Simpan selections lama jika keepSelections true
				local oldSelections = {}
				if keepSelections then
					oldSelections = DropdownSettings.CurrentOption
					-- Filter hanya yang masih ada di options baru
					local validSelections = {}
					for _, selection in ipairs(oldSelections) do
						if table.find(optionsTable, selection) then
							table.insert(validSelections, selection)
						end
					end
					DropdownSettings.CurrentOption = validSelections
				else
					DropdownSettings.CurrentOption = {}
				end
