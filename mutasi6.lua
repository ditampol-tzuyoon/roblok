--[[
    Script: Pet Mutation Bot (Perbaikan Final)
    Pembaruan:
    - Memperbaiki fungsi FindPetForMutation untuk membersihkan teks umur
      sebelum mengubahnya menjadi angka, mengatasi masalah `tonumber()`.
]]
...
-- == FUNGSI PENCARIAN PET YANG SUDAH DIPERBAIKI ==
    ...
    for _, petFrame in ipairs(petScrollingFrame:GetChildren()) do
        if petFrame:IsA("Frame") and petFrame:FindFirstChild("Main") then
            ...
            if petAgeLabel and petTypeLabel then
                -- Langkah 1: Ambil teks mentah
                local petAgeText = petAgeLabel.Text
                
                -- Langkah 2: Bersihkan teks, buang semua yang bukan angka
                local cleanedAgeText = string.gsub(petAgeText, "%D", "") -- %D berarti "non-digit"
                
                -- Langkah 3: Ubah teks yang sudah bersih menjadi angka
                local petAgeNum = tonumber(cleanedAgeText) or 0
                
                local petTypeText = petTypeLabel.Text:lower()

                -- Kondisi pengecekan tetap sama
                if petAgeNum >= 50 and not string.find(petTypeText, "rainbow") and not string.find(petTypeText, "golden") then
                    return petFrame.Name -- Ditemukan!
                end
            end
        end
    end
    return nil -- Tidak ada yang cocok
end
...
function LoadGameObjects()
    ...
    petModelocation = mutationMachine:WaitForChild("PetModelLocation", 10); if not petModelocation then return false, "Lokasi model pet tidak ditemukan." end
        ...
        while task.wait(1) do
            ...
            if botActive then
                if targetPetUUID and timerText:upper() == "READY" then
                    ...
                    if petModelocation:FindFirstChild("Dilophosaurus") then
