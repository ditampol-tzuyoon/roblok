--[[
    Script: Pet Mutation Bot (Debug Pencarian Pet)
    Pembaruan:
    - Menambahkan print statement detail di dalam fungsi FindPetForMutation
      untuk melacak proses pengambilan keputusan skrip.
]]
    ...
    for _, petFrame in ipairs(petScrollingFrame:GetChildren()) do
        if petFrame:IsA("Frame") and petFrame:FindFirstChild("Main") then
            ...
            if petAgeLabel and petTypeLabel then
                local petAgeText = petAgeLabel.Text
                local petAgeNum = tonumber(petAgeText) or 0
                local petTypeText = petTypeLabel.Text:lower()
                
                print("[DEBUG Pet] UUID: " .. petUUID .. " | Umur Teks: '" .. petAgeText .. "' | Umur Angka: " .. petAgeNum .. " | Tipe: '" .. petTypeText .. "'")

                if petAgeNum < 50 then
                    print("  -> DILEWATI: Umur di bawah 50.")
                    continue -- Lanjut ke pet berikutnya
                end

                if string.find(petTypeText, "rainbow") or string.find(petTypeText, "golden") then
                    print("  -> DILEWATI: Tipe adalah Rainbow atau Golden.")
                    continue -- Lanjut ke pet berikutnya
                end
                
                -- Jika lolos semua pemeriksaan
                print("  -> DITEMUKAN! Pet ini memenuhi syarat.")
                return petUUID
            else
                print("[DEBUG Pet] DILEWATI: Pet frame dengan UUID '"..petUUID.."' tidak memiliki label Umur/Tipe.")
            end
        end
    end
    print("--- Pengecekan Selesai, tidak ada pet yang memenuhi syarat ditemukan. ---")
    return nil -- Tidak ada pet yang cocok
end
...
if not success then
    ...
    coroutine.wrap(function()
        while task.wait(1) do
            ...
            local targetPetUUID = nil
            if botActive then -- Hanya cari pet jika bot aktif untuk menghemat resource
                targetPetUUID = FindPetForMutation()
            end
            ...
            if botActive then
                if targetPetUUID and timerText:upper() == "READY" then
                    botActive = false
                    print("Kondisi terpenuhi: Mesin 'READY' dan ada target pet. Memulai siklus...")
                    if petModelLocation:FindFirstChild("Dilophosaurus") then
                        print("Mengklaim pet dari mesin...")
                        petMutationEvent:FireServer("ClaimMutatedPet"); task.wait(3)
                    end
                    print("Mengambil target pet dari farm...")
                    petsServiceEvent:FireServer("UnequipPet", targetPetUUID); task.wait(3)
                    print("Mencari & menempatkan pet baru ke farm...")
                    local newFarmPet = FindPetToPlaceOnFarm()
                    if newFarmPet then petsServiceEvent:FireServer("EquipPet", newFarmPet) end
                    task.wait(3)
                    print("Memasukkan pet ke mesin dan memulai...")
                    petMutationEvent:FireServer("SubmitHeldPet"); task.wait(1)
                    petMutationEvent:FireServer("StartMachine")
                    cyclesCompleted = cyclesCompleted + 1
                    print("Siklus selesai. Menunggu kondisi berikutnya..."); task.wait(5)
                    botActive = true
                end
            end
        end
