--[[
    Script: Pet Mutation Bot (Kondisi "READY")
    Pembaruan:
    - Pemicu utama siklus diubah dari timer "00:00" menjadi teks "READY".
    - Tetap menggunakan pencarian cerdas untuk menemukan 'Part' yang benar.
]]
...
function LoadGameObjects()
    ...
    local billboardPart = machinePart:WaitForChild("BillboardPart", 5)
...
if not success then
    ...
    coroutine.wrap(function()
        while task.wait(1) do
            ...
            if botActive then
                -- [[ PERUBAHAN KONDISI UTAMA ]]
                -- Bot hanya akan berjalan jika ada pet yang siap (Age 50+)
                -- DAN jika teks pada mesin adalah "READY" (tidak peduli huruf besar/kecil)
                if targetPetUUID and timerText:upper() == "READY" then
                    botActive = false -- Jeda bot sementara agar tidak mengulang aksi
                    ...
                    print("Siklus selesai. Menunggu kondisi berikutnya...")
                    task.wait(5) -- Beri jeda tambahan setelah siklus selesai
                    botActive = true -- Aktifkan kembali bot
                end
            end
        end
    end)()
