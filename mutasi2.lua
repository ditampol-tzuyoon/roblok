--[[
    Script: Pet Mutation Bot (Versi Debugging Final)
    Tujuan: Mendapatkan laporan pasti tentang isi folder 'Part' dari sudut pandang skrip.
]]
...
function LoadGameObjects()
    ...
    -- Menggunakan 'workspace' (huruf kecil) sebagai percobaan, karena terkadang memiliki perilaku berbeda di executor
    ...
    -- == BAGIAN DEBUGGING KUNCI ==
    local billboardPart = machinePart:WaitForChild("BillboardPart", 20) -- Waktu tunggu dikembalikan ke 20 detik
    if not billboardPart then
        warn("DEBUG: Gagal menemukan 'BillboardPart' setelah 20 detik.")
        local children = machinePart:GetChildren()
        local childrenNames = {}
        if #children == 0 then
            table.insert(childrenNames, "KOSONG")
        else
            for _, child in ipairs(children) do
                table.insert(childrenNames, "'" .. child.Name .. "' (" .. child.ClassName .. ")") -- Tambahkan juga ClassName
            end
        end
        -- INI ADALAH LAPORAN YANG SAYA BUTUHKAN
        warn("DEBUG: Isi dari 'Part' adalah: " .. table.concat(childrenNames, ", "))
        return false, "Path rusak di '...:WaitForChild(\"BillboardPart\")'." 
    end
...
-- Sisa fungsi bot (FindPetForMutation, dll.) tidak perlu diubah dan sengaja tidak disertakan agar lebih ringkas
